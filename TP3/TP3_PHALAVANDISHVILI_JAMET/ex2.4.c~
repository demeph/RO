...
{	
	...
	glp_set_prob_name(prob, "Pers/Machines"); /* affectation d'un nom (on pourrait mettre NULL) */
	glp_set_obj_dir(prob, GLP_MAX); /* Il s'agit d'un problème de maximisation, on utiliserait la constante GLP_MIN dans le cas contraire */
	...
	for(i=1;i<=p.nbcontr;i++)
	{
		...
		if (i < 7) {
			...
			strcat(nomcontr[i-1], numero[i-1]); /* Les contraintes sont nommés "persMachine1", "persMachine2"... */		
		...
		else
		{
			...
			strcat(nomcontr[i-1], numero[i-1]); /* Les contraintes sont nommés "machinePers1", "machinePers2"... */		
			glp_set_row_name(prob, i, nomcontr[i-1]); /* Affectation du nom à la contrainte i */	
		}
	...
	for(i=1;i<=p.nbvar;i++)
	{
		...
		glp_set_col_name(prob, i , nomvar[i-1]); /* Les variables sont nommées "x(1,1)", "x(1,2)"... afin de respecter les noms de variables de l'exercice 2.4 */
	...
	glp_write_lp(prob,NULL,"persMachine.lp");
	...
	printf("z = %lf\n",z);
	for(i = 0;i < p.nbvar;i++) printf("x%d = %d\n",(i+1),(int)(x[i] + 0.5)); /* un cast est ajouté, x[i] pourrait être égal à 0.99999... */ 
	//puts("");
